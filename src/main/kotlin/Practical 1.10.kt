    class Car(type:String, model:String, owner:String, miles:Int, price:Int, cprice:Int){
        var ty:String = type
        var mo =model
        var ow = owner
        var m:Int = miles
        var p:Int = price
        var cp:Int = cprice

        fun getCarPrice(){
            println("Car Information: $ty , $mo\nCar Owner: $ow\nMiles Driven: $m\nOriginal Car Price: $p, Current Car Price: $cp ")
        }
    }
    fun main() {

        var cars = arrayOf(Car("BMW","2018","Aryan",105,100000, 98950),
                Car("BMW","2019","Krish",20,400000, 399800),
                Car("Toyota","2017","ABC",100,1080000, 1079000),
                Car("Maruti", "2020", "XYZ", 200, 4000000, 3998000))
        println("Creating Car Class Object car1 in next line")
        println("Object of class is created and Init is called.\n-----")
        for (i in cars){
            i.getCarPrice()
        }
    }

